    ...
    <div className="container-editorial py-10 md:py-16">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="space-y-6">
          ...
          {order.notes?.includes("Motoboy Próprio") && (
            <div className="p-5 bg-[#25D366]/10 border border-[#25D366]/30 rounded-md">
              <h3 className="font-display text-lg mb-2 flex items-center gap-2 text-[#128C7E]">
                <Bike className="h-5 w-5" /> Entrega via Motoboy
              </h3>
              <p className="text-sm text-muted-foreground mb-4">
                Como você escolheu a entrega via motoboy, por favor, clique no botão abaixo para
                combinar o horário de entrega pelo WhatsApp.
              </p>
              <a
                href={`https://wa.me/${(shippingSettings as Record<string, any>)?.whatsapp_number || "5511994565923"}?text=${encodeURIComponent(`Olá! Fiz o pedido #${order.order_number} no site e escolhi a entrega via Motoboy Próprio. Gostaria de combinar o horário de entrega.`)}`}
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-2 bg-[#25D366] text-white px-4 py-2.5 rounded-sm text-sm font-medium hover:bg-[#128C7E] transition-colors"
              >
                <MessageCircle className="h-4 w-4" /> Combinar Entrega
              </a>
            </div>
          )}
