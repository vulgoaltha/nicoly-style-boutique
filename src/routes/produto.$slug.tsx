...
import { Minus, Plus, ShoppingBag, Heart, Truck, ChevronLeft } from "lucide-react";
...
function ProductPage() {
  ...
  if (isLoading) {
    return (
      <div className="container-editorial py-10 md:py-16 grid md:grid-cols-2 gap-8 md:gap-12">
  ...
  return (
    <div className="container-editorial py-6 md:py-16">
      <script type="application/ld+json">{JSON.stringify(schemaOrgProduct)}</script>

      {/* Breadcrumb */}
      <nav className="text-xs text-muted-foreground mb-5 tracking-editorial uppercase flex items-center gap-1 flex-wrap">
      ...
      <div className="grid md:grid-cols-2 gap-6 md:gap-12">
            ...
            <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
              {product.images.map((img, i) => (
                <button
                  key={i}
                  onClick={() => setImgIdx(i)}
                  className={`flex-shrink-0 w-16 h-20 sm:w-20 sm:h-24 overflow-hidden rounded-sm border-2 transition ${
        ...
        <div className="space-y-5">
          <div>
            <h1 className="font-display text-2xl sm:text-3xl md:text-4xl">{product.name}</h1>
              ...
              <div className="flex flex-wrap gap-2">
                {product.colors.map((c) => (
                  <button
                    key={c}
                    onClick={() => setColor(c)}
                    className={`px-3 py-2 min-h-[44px] text-xs border rounded-sm transition ${
              ...
              <div className="flex flex-wrap gap-2">
                {product.sizes.map((s) => (
                  <button
                    key={s}
                    onClick={() => setSize(s)}
                    className={`min-w-[44px] min-h-[44px] px-3 py-2 text-xs border rounded-sm transition ${
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="flex-1 bg-primary text-primary-foreground px-6 h-11 text-xs tracking-editorial uppercase rounded-sm hover:bg-blush-deep transition disabled:opacity-40 inline-flex items-center justify-center gap-2"
            >
              <ShoppingBag className="h-4 w-4" />
              {product.stock === 0 ? "Esgotado" : "Adicionar à sacola"}
            </button>
            <button
              className="flex h-11 w-11 items-center justify-center border border-border rounded-sm hover:text-blush transition"
