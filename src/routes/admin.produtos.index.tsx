          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead className="bg-secondary/50 text-xs tracking-editorial uppercase text-muted-foreground">
              <tr>
                ...
                <th className="p-3 w-[100px] text-right">Ações</th>
              </tr>
            </thead>
            <tbody>
              {products.map((p) => (
                ...
                >
                  <td className="p-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium text-sm whitespace-normal max-w-[200px] sm:max-w-xs line-clamp-2">
                          {p.name}
                        </div>
                  ...
                  <td className="p-3 text-sm">{brl(p.sale_price ?? p.price)}</td>
                  <td className={`p-3 text-sm ${p.stock === 0 ? "text-destructive" : ""}`}>
                    {p.stock}
                  </td>
                  <td className="p-3">
                    <span
                  ...
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-2">
                      <Link
                        ...
                        params={{ id: p.id }}
                        className="p-2 hover:bg-secondary rounded flex items-center justify-center transition"
                      >
                        <Edit className="h-4 w-4" />
                      </Link>
                      <button
                        onClick={() => remove(p.id)}
                        className="p-2 hover:bg-secondary rounded text-destructive flex items-center justify-center transition"
                      >
