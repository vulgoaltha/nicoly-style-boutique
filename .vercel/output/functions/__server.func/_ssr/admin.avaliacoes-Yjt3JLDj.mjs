import { r as N, j as e } from "../_libs/react.mjs";
import { b as y, a as k, u as x } from "../_libs/tanstack__react-query.mjs";
  ...
  return e.jsx("div", {
    ...
    children: Array.from({ length: 5 }).map((i, o) =>
      e.jsx(
        E,
        {
          className: `h-3.5 w-3.5 ${o < r ? "fill-amber-400 text-amber-400" : "fill-border text-border"}`,
        },
        o,
      ),
    ),
...
function Y() {
  const r = y(),
    [i, o] = N.useState("all"),
    { data: t = [], isLoading: d } = k({
      queryKey: ["admin-reviews"],
      queryFn: async () => {
        const { data: a, error: s } = await p
    ...
    m = x({
      mutationFn: async (a) => {
        const { error: s } = await p.from("customer_reviews").update({ approved: !0 }).eq("id", a);
        if (s) throw s;
      },
      onSuccess: () => {
        (n.success("Avaliação aprovada!"),
          r.invalidateQueries({ queryKey: ["admin-reviews"] }),
          r.invalidateQueries({ queryKey: ["public-reviews"] }));
      },
      onError: () => n.error("Erro ao aprovar avaliação."),
    }),
    l = x({
      mutationFn: async (a) => {
        const { error: s } = await p.from("customer_reviews").update({ approved: !1 }).eq("id", a);
        if (s) throw s;
      },
      onSuccess: () => {
        (n.success("Avaliação rejeitada."),
          r.invalidateQueries({ queryKey: ["admin-reviews"] }),
          r.invalidateQueries({ queryKey: ["public-reviews"] }));
      },
      onError: () => n.error("Erro ao rejeitar avaliação."),
    }),
    b = x({
      mutationFn: async (a) => {
        const { error: s } = await p.from("customer_reviews").delete().eq("id", a);
        if (s) throw s;
      },
      onSuccess: () => {
        (n.success("Avaliação excluída."),
          r.invalidateQueries({ queryKey: ["admin-reviews"] }),
          r.invalidateQueries({ queryKey: ["public-reviews"] }));
      },
      onError: () => n.error("Erro ao excluir avaliação."),
    }),
    g = t.filter((a) =>
      i === "approved" ? a.approved === !0 : i === "pending" ? a.approved === !1 : !0,
    ),
    h = t.filter((a) => a.approved).length,
    v = t.filter((a) => !a.approved).length,
    w = t.length ? (t.reduce((a, s) => a + s.rating, 0) / t.length).toFixed(1) : "—";
  return e.jsxs("div", {
    children: [
      e.jsxs("div", {
        ...
        children: [
          ...
          e.jsxs("div", {
            ...
            children: [
              e.jsx(u, { label: "Aprovadas", value: h, color: "text-green-600" }),
              e.jsx(u, { label: "Pendentes", value: v, color: "text-amber-600" }),
              e.jsx(u, { label: "Nota média", value: w, color: "text-blush-deep" }),
            ],
          }),
        ],
      }),
      e.jsx("div", {
        ...
        children: ["all", "pending", "approved"].map((a) => {
          const s = {
            all: `Todas (${t.length})`,
            pending: `Pendentes (${v})`,
            approved: `Aprovadas (${h})`,
          };
          return e.jsx(
            "button",
            {
              onClick: () => o(a),
              className: `text-xs tracking-editorial uppercase px-3 py-1.5 rounded-sm transition ${i === a ? "bg-primary text-primary-foreground" : "hover:bg-secondary text-muted-foreground"}`,
              children: s[a],
            },
            a,
          );
      ...
      d
        ...
        : g.length === 0
          ? e.jsxs("div", {
              ...
              children: [
                e.jsx(C, { className: "h-10 w-10 text-border" }),
              ...
              children: g.map((a) =>
                e.jsx(
                  Q,
                  {
                    ...
                    onReject: () => l.mutate(a.id),
                    onDelete: () => b.mutate(a.id),
                    isLoading: m.isPending || l.isPending || b.isPending,
                  },
                  a.id,
                ),
              ),
...
function Q({ review: r, onApprove: i, onReject: o, onDelete: t, isLoading: d }) {
            ...
            children: [
              ...
              r.product &&
                e.jsxs("span", {
                  ...
                  children: [e.jsx(A, { className: "h-2.5 w-2.5" }), r.product.name],
                }),
            ],
          }),
        ],
        ...
        children: [
          ...
          m
            ...
            : e.jsx(c, {
                ...
                children: e.jsx(_, { className: "h-3.5 w-3.5" }),
              }),
        ],
      }),
    ],
...
function c({ children: r, onClick: i, disabled: o, title: t, className: d = "" }) {
  return e.jsx("button", {
    onClick: i,
    disabled: o,
    title: t,
