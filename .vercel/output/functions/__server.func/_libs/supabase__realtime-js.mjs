import { P as H, S as I } from "./supabase__phoenix.mjs";
class M {
  constructor() {}
...
const J = "2.108.1",
  ...
  T = { connecting: "connecting", closing: "closing", closed: "closed" };
class K {
  constructor(e) {
  ...
  _encodeBinaryUserBroadcastPush(e) {
    var t, r;
    const s =
      (r = (t = e.payload) === null || t === void 0 ? void 0 : t.payload) !== null && r !== void 0
        ? r
        : new ArrayBuffer(0);
  ...
  _encodeUserBroadcastPush(e, t, r) {
    ...
    const o = e.topic,
      ...
      d = e.payload.event,
      p = this.allowedMetadataKeys ? this._pick(e.payload, this.allowedMetadataKeys) : {},
      f = Object.keys(p).length === 0 ? "" : JSON.stringify(p);
    ...
    if (d.length > 255) throw new Error(`userEvent length ${d.length} exceeds maximum of 255`);
    if (f.length > 255) throw new Error(`metadata length ${f.length} exceeds maximum of 255`);
    const g =
        this.USER_BROADCAST_PUSH_META_LENGTH + c.length + a.length + o.length + d.length + f.length,
      h = new ArrayBuffer(this.HEADER_LENGTH + g);
    let l = new DataView(h),
      v = 0;
    (l.setUint8(v++, this.KINDS.userBroadcastPush),
      l.setUint8(v++, c.length),
      l.setUint8(v++, a.length),
      l.setUint8(v++, o.length),
      l.setUint8(v++, d.length),
      l.setUint8(v++, f.length),
      l.setUint8(v++, t),
      Array.from(c, (m) => l.setUint8(v++, m.charCodeAt(0))),
      Array.from(a, (m) => l.setUint8(v++, m.charCodeAt(0))),
      Array.from(o, (m) => l.setUint8(v++, m.charCodeAt(0))),
      Array.from(d, (m) => l.setUint8(v++, m.charCodeAt(0))),
      Array.from(f, (m) => l.setUint8(v++, m.charCodeAt(0))));
    var _ = new Uint8Array(h.byteLength + r.byteLength);
    return (_.set(new Uint8Array(h), 0), _.set(new Uint8Array(r), h.byteLength), _.buffer);
  }
  decode(e, t) {
    ...
    if (typeof e == "string") {
      const r = JSON.parse(e),
        [s, i, o, a, c] = r;
      return t({ join_ref: s, ref: i, topic: o, event: a, payload: c });
    }
    return t({});
  }
  _binaryDecode(e) {
  ...
  _decodeUserBroadcast(e, t, r) {
    ...
    const d = r.decode(e.slice(c, c + s));
    c = c + s;
    const p = r.decode(e.slice(c, c + i));
    c = c + i;
    const f = r.decode(e.slice(c, c + o));
    c = c + o;
    const g = e.slice(c, e.byteLength),
      h = a === this.JSON_ENCODING ? JSON.parse(r.decode(g)) : g,
      l = { type: this.BROADCAST_EVENT, event: p, payload: h };
    return (
      o > 0 && (l.meta = JSON.parse(f)),
      { join_ref: null, ref: null, topic: d, event: this.BROADCAST_EVENT, payload: l }
    );
  }
  _isArrayBuffer(e) {
    var t;
...
const R = (n, e, t = {}) => {
    ...
    return e ? Object.keys(e).reduce((i, o) => ((i[o] = G(o, n, e, s)), i), {}) : {};
  },
  G = (n, e, t, r) => {
    const s = e.find((a) => a.name === n),
      i = s?.type,
      o = t[n];
    return i && !r.includes(i) ? L(i, o) : O(o);
  },
  L = (n, e) => {
    if (n.charAt(0) === "_") {
  ...
  Z = (n, e) => {
    ...
    if (n[0] === "{" && r === "}") {
      ...
      return i.map((a) => L(e, a));
    }
    return n;
  },
  Q = (n) => (typeof n == "string" ? n.replace(" ", "T") : n),
  W = (n) => {
    const e = new URL(n);
    return (
...
class A {
  constructor(e, t) {
    const r = te(t);
    ((this.presence = new H(e.getChannel(), r)),
      this.presence.onJoin((s, i, o) => {
        const a = A.onJoinPayload(s, i, o);
        e.getChannel().trigger("presence", a);
      }),
      this.presence.onLeave((s, i, o) => {
        const a = A.onLeavePayload(s, i, o);
        e.getChannel().trigger("presence", a);
      }),
  ...
  get state() {
    return A.transformState(this.presence.state);
  }
...
class ne {
  ...
  constructor(e, t) {
    ((this.channel = e), (this.presenceAdapter = new A(this.channel.channelAdapter, t)));
  }
}
function re(n) {
  ...
  if (n && typeof n == "object") {
    ...
    if (typeof e.code == "number") {
      ...
      return new Error(`socket closed: ${e.code}${t}`, { cause: n });
    }
...
class se {
  ...
  push(e, t, r) {
    ...
    if (this.channel.pushBuffer.length > F) {
...
class E {
  ...
  constructor(e, t = { config: {} }, r) {
    ...
    if (
      ...
      (this.channelAdapter = new se(this.socket.socketAdapter, e, this.params)),
      (this.presence = new ne(this)),
      this._onClose(() => {
        this.socket._remove(this);
      }),
      this._updateFilterTransform(),
      (this.broadcastEndpointURL = W(this.socket.socketAdapter.endPointURL())),
      (this.private = this.params.config.private || !1),
      !this.private &&
        !(
          (i = (s = this.params.config) === null || s === void 0 ? void 0 : s.broadcast) === null ||
          i === void 0
        ) &&
        i.replay)
    )
      ...
      (this.socket.accessTokenValue && (f.access_token = this.socket.accessTokenValue),
        ...
        this.channelAdapter
          ...
          .receive("error", (h) => {
            ...
            const l = Object.values(h).join(", ") || "error";
            e?.(k.CHANNEL_ERROR, new Error(l, { cause: h }));
          })
          .receive("timeout", () => {
            e?.(k.TIMED_OUT);
          }));
  ...
  _updatePostgresBindings(e, t) {
    ...
    const s = this.bindings.postgres_changes,
      i = (r = s?.length) !== null && r !== void 0 ? r : 0,
      o = [];
    for (let a = 0; a < i; a++) {
      const c = s[a],
        {
          filter: { event: d, schema: p, table: f, filter: g },
        } = c,
        h = e && e[a];
      if (
        h &&
        h.event === d &&
        E.isFilterValueEqual(h.schema, p) &&
        E.isFilterValueEqual(h.table, f) &&
        E.isFilterValueEqual(h.filter, g)
      )
        o.push(Object.assign(Object.assign({}, c), { id: h.id }));
      else {
        (this.unsubscribe(),
          (this.state = b.errored),
          t?.(
    ...
    ((this.bindings.postgres_changes = o), this.state != b.errored && t && t(k.SUBSCRIBED));
  }
  presenceState() {
  ...
  on(e, t, r) {
    const s = this.channelAdapter.isJoined() || this.channelAdapter.isJoining(),
      i = e === w.PRESENCE || e === w.POSTGRES_CHANGES;
    if (s && i)
      throw (
        this.socket.log(
          "channel",
  ...
  async httpSend(e, t, r = {}) {
    ...
    const c = { method: "POST", headers: o, body: i ? t : JSON.stringify(t) },
  ...
  async send(e, t = {}) {
    ...
    if (!this.channelAdapter.canPush() && e.type === "broadcast") {
      ...
      try {
        ...
        return (
          await ((s = d.body) === null || s === void 0 ? void 0 : s.cancel()),
          d.ok ? "ok" : "error"
        );
      } catch (d) {
        return d instanceof Error && d.name === "AbortError" ? "timed out" : "error";
      }
    } else
      return new Promise((i) => {
        var o, a, c;
        const d = this.channelAdapter.push(e.type, e, t.timeout || this.timeout);
        (e.type === "broadcast" &&
          !(
            !(
              (c =
  ...
  async _fetchWithTimeout(e, t, r) {
    const s = new AbortController(),
      ...
      o = await this.socket.fetch(e, Object.assign(Object.assign({}, t), { signal: s.signal }));
    return (clearTimeout(i), o);
  }
  _on(e, t, r) {
    const s = e.toLocaleLowerCase(),
      i = this.channelAdapter.on(e, r),
      o = { type: s, filter: t, callback: r, ref: i };
    return (
  ...
  _updateFilterMessage() {
    this.channelAdapter.updateFilterBindings((e, t, r) => {
      ...
      if (this._notThisChannelEvent(f, r)) return !1;
      const g =
        (s = this.bindings[f]) === null || s === void 0 ? void 0 : s.find((h) => h.ref === e.ref);
      if (!g) return !0;
      if (["broadcast", "presence", "postgres_changes"].includes(f))
        if ("id" in g) {
          const h = g.id,
            l = (i = g.filter) === null || i === void 0 ? void 0 : i.event;
          return (
            h &&
            ((o = t.ids) === null || o === void 0 ? void 0 : o.includes(h)) &&
            (l === "*" ||
              l?.toLocaleLowerCase() ===
                ((a = t.data) === null || a === void 0 ? void 0 : a.type.toLocaleLowerCase()))
          );
        } else {
          const h =
            (d = (c = g?.filter) === null || c === void 0 ? void 0 : c.event) === null ||
            d === void 0
              ? void 0
              : d.toLocaleLowerCase();
          return (
            h === "*" ||
            h === ((p = t?.event) === null || p === void 0 ? void 0 : p.toLocaleLowerCase())
          );
        }
      else return g.type.toLocaleLowerCase() === f;
    });
  }
  _notThisChannelEvent(e, t) {
    const { close: r, error: s, leave: i, join: o } = D;
    return t && [r, s, i, o].includes(e) && t !== this.joinPush.ref;
  }
  _updateFilterTransform() {
    this.channelAdapter.updatePayloadTransform((e, t, r) => {
      if (typeof t == "object" && "ids" in t) {
        const s = t.data,
          { schema: i, table: o, commit_timestamp: a, type: c, errors: d } = s;
        return Object.assign(
          Object.assign(
            {},
            { schema: i, table: o, commit_timestamp: a, eventType: c, new: {}, old: {}, errors: d },
          ),
  ...
  copyBindings(e) {
    ...
    for (const t in e.bindings)
      for (const r of e.bindings[t]) this._on(r.type, r.filter, r.callback);
  }
  ...
  _getPayloadRecords(e) {
    const t = { new: {}, old: {} };
    return (
      (e.type === "INSERT" || e.type === "UPDATE") && (t.new = R(e.columns, e.record)),
      (e.type === "UPDATE" || e.type === "DELETE") && (t.old = R(e.columns, e.old_record)),
      t
    );
  }
}
...
const P = { HEARTBEAT_INTERVAL: 25e3 },
  ae = [1e3, 2e3, 5e3, 1e4],
  ce = 1e4;
function le() {
  const n = new Map();
  return {
    ...
    key(e) {
      var t;
      return (t = Array.from(n.keys())[e]) !== null && t !== void 0 ? t : null;
    },
    removeItem(e) {
      n.delete(e);
    },
...
Option 2: Install and provide the "ws" package:
  ...
  channel(e, t = { config: {} }) {
    ...
    if (s) return s;
    {
      const i = new E(`realtime:${e}`, t, this);
    ...
    (r ? (this._manuallySetToken = !0) : this.accessToken && (this._manuallySetToken = !1),
      this.accessTokenValue != t &&
        ...
        this.channels.forEach((s) => {
          const i = { access_token: t, version: $ };
          (t && s.updateJoinPayload(i),
            s.joinedOnce &&
              s.channelAdapter.isJoined() &&
              s.channelAdapter.push(D.access_token, { access_token: t }));
        })));
  }
  ...
  _workerObjectUrl(e) {
    ...
    if (e) t = e;
      ...
      const r = new Blob([he], { type: "application/javascript" });
      t = URL.createObjectURL(r);
    }
    return t;
  }
  _initializeOptions(e) {
    var t, r, s, i, o, a, c, d, p, f, g, h;
    ((this.worker = (t = e?.worker) !== null && t !== void 0 ? t : !1),
      (this.accessToken = (r = e?.accessToken) !== null && r !== void 0 ? r : null));
    const l = {};
    ((l.timeout = (s = e?.timeout) !== null && s !== void 0 ? s : z),
      (l.heartbeatIntervalMs =
        (i = e?.heartbeatIntervalMs) !== null && i !== void 0 ? i : P.HEARTBEAT_INTERVAL),
      (this._disconnectOnEmptyChannelsAfterMs =
        (o = e?.disconnectOnEmptyChannelsAfterMs) !== null && o !== void 0
          ? o
          : 2 * ((a = e?.heartbeatIntervalMs) !== null && a !== void 0 ? a : P.HEARTBEAT_INTERVAL)),
      (l.transport = (c = e?.transport) !== null && c !== void 0 ? c : M.getWebSocketConstructor()),
      (l.params = e?.params),
      (l.logger = e?.logger),
      (l.heartbeatCallback = this._wrapHeartbeatCallback(e?.heartbeatCallback)),
      (l.sessionStorage = (d = e?.sessionStorage) !== null && d !== void 0 ? d : de()),
      (l.reconnectAfterMs =
        (p = e?.reconnectAfterMs) !== null && p !== void 0 ? p : (y) => ae[y - 1] || ce));
    let v, _;
    const m = (f = e?.vsn) !== null && f !== void 0 ? f : V;
    switch (m) {
      case x:
        ((v = (y, S) => S(JSON.stringify(y))), (_ = (y, S) => S(JSON.parse(y))));
        break;
      case B:
    ...
    if (
      ((l.vsn = m),
      (l.encode = (g = e?.encode) !== null && g !== void 0 ? g : v),
      (l.decode = (h = e?.decode) !== null && h !== void 0 ? h : _),
      (l.beforeReconnect = this._reconnectAuth.bind(this)),
      (e?.logLevel || e?.log_level) &&
        ((this.logLevel = e.logLevel || e.log_level),
        (l.params = Object.assign(Object.assign({}, l.params), { log_level: this.logLevel }))),
      this.worker)
    ) {
