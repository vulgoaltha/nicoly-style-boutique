import { I as V } from "./iceberg-js.mjs";
function v(t) {
...
function c(t) {
  for (var e = 1; e < arguments.length; e++) {
    ...
    e % 2
      ? j(Object(r), !0).forEach(function (a) {
          D(t, a, r[a]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(t, Object.getOwnPropertyDescriptors(r))
        : j(Object(r)).forEach(function (a) {
...
function b(t, e, r) {
  const a = c({}, t),
    n = e.toLowerCase();
  for (const s of Object.keys(a)) s.toLowerCase() === n && delete a[s];
  return ((a[n] = r), a);
}
function R(t) {
  const e = {};
  for (const [r, a] of Object.entries(t)) e[r.toLowerCase()] = a;
  return e;
}
const A = (t) => (t ? (...e) => t(...e) : (...e) => fetch(...e)),
  H = (t) => {
  ...
  $ = (t) => {
    if (Array.isArray(t)) return t.map((r) => $(r));
    if (typeof t == "function" || t !== Object(t)) return t;
    const e = {};
    return (
      Object.entries(t).forEach(([r, a]) => {
        const n = r.replace(/([-_][a-z])/gi, (s) => s.toUpperCase().replace(/[-_]/g, ""));
        e[n] = $(a);
      }),
      e
    );
  ...
  z = async (t, e, r, a) => {
    if (t !== null && typeof t == "object" && "json" in t && typeof t.json == "function") {
      const n = t;
      let s = parseInt(String(n.status), 10);
      (Number.isFinite(s) || (s = 500),
        n
          .json()
          .then((o) => {
            const u = o?.statusCode || o?.code || s + "";
            e(new x(T(o), s, u, a));
          })
          .catch(() => {
            const o = s + "";
            e(new x(n.statusText || `HTTP ${s} error`, s, o, a));
          }));
    } else e(new C(T(t), t, a));
  },
  K = (t, e, r, a) => {
    const n = { method: t, headers: e?.headers || {} };
    if (t === "GET" || t === "HEAD" || !a) return c(c({}, n), r);
    if (H(a)) {
      var s;
      const o = e?.headers || {};
      let u;
      for (const [i, l] of Object.entries(o)) i.toLowerCase() === "content-type" && (u = l);
      ((n.headers = b(
        o,
        "Content-Type",
        (s = u) !== null && s !== void 0 ? s : "application/json",
      )),
        (n.body = JSON.stringify(a)));
    } else n.body = a;
    return (e?.duplex && (n.duplex = e.duplex), c(c({}, n), r));
  };
async function g(t, e, r, a, n, s, o) {
...
function I(t = "storage") {
  return {
    get: async (e, r, a, n) => g(e, "GET", r, a, n, void 0, t),
    post: async (e, r, a, n, s) => g(e, "POST", r, n, s, a, t),
    put: async (e, r, a, n, s) => g(e, "PUT", r, n, s, a, t),
    head: async (e, r, a, n) =>
      g(e, "HEAD", r, c(c({}, a), {}, { noResolveJson: !0 }), n, void 0, t),
    remove: async (e, r, a, n, s) => g(e, "DELETE", r, n, s, a, t),
  };
}
const M = I("storage"),
  { get: m, post: f, put: B, head: J, remove: P } = M,
  h = I("vectors");
var p = class {
  constructor(t, e = {}, r, a = "storage") {
  ...
  setHeader(t, e) {
    return ((this.headers = b(this.headers, t, e)), this);
  }
...
const W = { limit: 100, offset: 0, sortBy: { column: "name", order: "asc" } },
...
var X = class extends p {
  ...
  async uploadOrUpdate(t, e, r, a) {
    ...
    return n.handleOperation(async () => {
      let s;
      const o = c(c({}, E), a);
      let u = c(c({}, n.headers), t === "POST" && { "x-upsert": String(o.upsert) });
      const i = o.metadata;
      if (
        (typeof Blob < "u" && r instanceof Blob
          ? ((s = new FormData()),
            s.append("cacheControl", o.cacheControl),
            i && s.append("metadata", n.encodeMetadata(i)),
            s.append("", r))
          : typeof FormData < "u" && r instanceof FormData
            ? ((s = r),
              s.has("cacheControl") || s.append("cacheControl", o.cacheControl),
              i && !s.has("metadata") && s.append("metadata", n.encodeMetadata(i)))
            : ((s = r),
              (u["cache-control"] = `max-age=${o.cacheControl}`),
              (u["content-type"] = o.contentType),
              i && (u["x-metadata"] = n.toBase64(n.encodeMetadata(i))),
        ...
        for (const [O, k] of Object.entries(a.headers)) u = b(u, O, k);
      const l = n._removeEmptyFolders(e),
        d = n._getFinalPath(l),
        y = await (t == "PUT" ? B : f)(
          n.fetch,
          `${n.url}/object/${d}`,
          s,
          c({ headers: u }, o?.duplex ? { duplex: o.duplex } : {}),
        );
      return { path: l, id: y.Id, fullPath: y.Key };
    });
  }
  async upload(t, e, r) {
  ...
  async uploadToSignedUrl(t, e, r, a) {
    var n = this;
    const s = n._removeEmptyFolders(t),
      o = n._getFinalPath(s),
      u = new URL(n.url + `/object/upload/sign/${o}`);
    return (
      u.searchParams.set("token", e),
      n.handleOperation(async () => {
        let i;
        const l = c(c({}, E), a);
        let d = c(c({}, n.headers), { "x-upsert": String(l.upsert) });
        const y = l.metadata;
        if (
          (typeof Blob < "u" && r instanceof Blob
            ? ((i = new FormData()),
              i.append("cacheControl", l.cacheControl),
              y && i.append("metadata", n.encodeMetadata(y)),
              i.append("", r))
            : typeof FormData < "u" && r instanceof FormData
              ? ((i = r),
                i.has("cacheControl") || i.append("cacheControl", l.cacheControl),
                y && !i.has("metadata") && i.append("metadata", n.encodeMetadata(y)))
              : ((i = r),
                (d["cache-control"] = `max-age=${l.cacheControl}`),
                (d["content-type"] = l.contentType),
                y && (d["x-metadata"] = n.toBase64(n.encodeMetadata(y))),
          ...
          for (const [O, k] of Object.entries(a.headers)) d = b(d, O, k);
        return {
          path: s,
          fullPath: (
            await B(
              ...
              u.toString(),
              i,
              c({ headers: d }, l?.duplex ? { duplex: l.duplex } : {}),
            )
          ).Key,
        };
      })
  ...
  async createSignedUploadUrl(t, e) {
    ...
    return r.handleOperation(async () => {
      let a = r._getFinalPath(t);
      const n = c({}, r.headers);
      e?.upsert && (n["x-upsert"] = "true");
      const s = await f(r.fetch, `${r.url}/object/upload/sign/${a}`, {}, { headers: n }),
        o = new URL(r.url + s.url),
        u = o.searchParams.get("token");
      if (!u) throw new _("No token returned by API");
  ...
  async createSignedUrl(t, e, r) {
    ...
    return a.handleOperation(async () => {
      ...
      let o = await f(
        a.fetch,
        `${a.url}/object/sign/${n}`,
        c({ expiresIn: e }, s ? { transform: r.transform } : {}),
        { headers: a.headers },
      );
      ...
      const i = u.toString();
      return { signedUrl: encodeURI(`${a.url}${o.signedURL}${i ? `&${i}` : ""}`) };
    });
  }
  async createSignedUrls(t, e, r) {
    ...
    return a.handleOperation(async () => {
      ...
      (r?.download && s.set("download", r.download === !0 ? "" : r.download),
        r?.cacheNonce != null && s.set("cacheNonce", String(r.cacheNonce)));
      const o = s.toString();
      return n.map((u) =>
        c(
          c({}, u),
          {},
          {
            signedUrl: u.signedURL ? encodeURI(`${a.url}${u.signedURL}${o ? `&${o}` : ""}`) : null,
          },
        ),
      );
    });
  ...
  download(t, e, r) {
    ...
    return new Q(u, this.shouldThrowOnError);
  }
  async info(t) {
    var e = this;
    const r = e._getFinalPath(t);
    return e.handleOperation(async () =>
      $(await m(e.fetch, `${e.url}/object/info/${r}`, { headers: e.headers })),
    );
  }
  async exists(t) {
    var e = this;
    ...
    try {
      return (
        await J(e.fetch, `${e.url}/object/${r}`, { headers: e.headers }),
        { data: !0, error: null }
      );
    } catch (n) {
      ...
      if (S(n)) {
        ...
        const s =
          n instanceof x
            ? n.status
            : n instanceof C
              ? (a = n.originalError) === null || a === void 0
                ? void 0
                : a.status
              : void 0;
        if (s !== void 0 && [400, 404].includes(s)) return { data: !1, error: n };
      }
      throw n;
    }
  }
  getPublicUrl(t, e) {
    ...
    (e?.download && a.set("download", e.download === !0 ? "" : e.download),
      e?.transform && this.applyTransformOptsToQuery(a, e.transform),
    ...
    const n = a.toString(),
      s =
        ...
        Object.keys(e.transform).length > 0
          ? "render/image"
          : "object";
    return { data: { publicUrl: encodeURI(`${this.url}/${s}/public/${r}`) + (n ? `?${n}` : "") } };
  }
  async remove(t) {
    var e = this;
    return e.handleOperation(
      async () =>
        await P(e.fetch, `${e.url}/object/${e.bucketId}`, { prefixes: t }, { headers: e.headers }),
    );
  }
  async list(t, e, r) {
    var a = this;
    return a.handleOperation(async () => {
      const n = c(c(c({}, W), e), {}, { prefix: t || "" });
      return await f(a.fetch, `${a.url}/object/list/${a.bucketId}`, n, { headers: a.headers }, r);
    });
  }
  async listV2(t, e) {
    var r = this;
...
var Z = class extends p {
    ...
    async listBuckets(t) {
      ...
      return e.handleOperation(async () => {
        ...
        return await m(e.fetch, `${e.url}/bucket${r}`, { headers: e.headers });
      });
    }
    async getBucket(t) {
      var e = this;
      return e.handleOperation(
        async () => await m(e.fetch, `${e.url}/bucket/${t}`, { headers: e.headers }),
      );
    }
    ...
    async emptyBucket(t) {
      var e = this;
      return e.handleOperation(
        async () => await f(e.fetch, `${e.url}/bucket/${t}/empty`, {}, { headers: e.headers }),
      );
    }
    async deleteBucket(t) {
      var e = this;
      return e.handleOperation(
        async () => await P(e.fetch, `${e.url}/bucket/${t}`, {}, { headers: e.headers }),
      );
    }
  ...
  ee = class extends p {
    ...
    async createBucket(t) {
      var e = this;
      return e.handleOperation(
        async () => await f(e.fetch, `${e.url}/bucket`, { name: t }, { headers: e.headers }),
      );
    }
    async listBuckets(t) {
      var e = this;
      return e.handleOperation(async () => {
        ...
        const a = r.toString(),
          n = a ? `${e.url}/bucket?${a}` : `${e.url}/bucket`;
        return await m(e.fetch, n, { headers: e.headers });
      });
    }
    async deleteBucket(t) {
      var e = this;
      ...
      return new Proxy(r, {
        get(n, s) {
          ...
          return typeof o != "function"
            ...
            : async (...u) => {
                try {
                  return { data: await o.apply(n, u), error: null };
                } catch (i) {
                  if (a) throw i;
  ...
  te = class extends p {
    constructor(t, e = {}, r) {
      const a = t.replace(/\/$/, ""),
        n = c(c({}, w), {}, { "Content-Type": "application/json" }, e);
      super(a, n, r, "vectors");
    }
    async createIndex(t) {
      var e = this;
      return e.handleOperation(
        async () =>
          (await h.post(e.fetch, `${e.url}/CreateIndex`, t, { headers: e.headers })) || {},
      );
    }
    async getIndex(t, e) {
      var r = this;
    ...
    async listIndexes(t) {
      var e = this;
      return e.handleOperation(
        async () => await h.post(e.fetch, `${e.url}/ListIndexes`, t, { headers: e.headers }),
      );
    }
  ...
  re = class extends p {
    ...
    async putVectors(t) {
      ...
      return e.handleOperation(
        async () => (await h.post(e.fetch, `${e.url}/PutVectors`, t, { headers: e.headers })) || {},
      );
    }
    async getVectors(t) {
      var e = this;
      return e.handleOperation(
        async () => await h.post(e.fetch, `${e.url}/GetVectors`, t, { headers: e.headers }),
      );
    }
    async listVectors(t) {
      var e = this;
      ...
      return e.handleOperation(
        async () => await h.post(e.fetch, `${e.url}/ListVectors`, t, { headers: e.headers }),
      );
    }
    async queryVectors(t) {
      var e = this;
      return e.handleOperation(
        async () => await h.post(e.fetch, `${e.url}/QueryVectors`, t, { headers: e.headers }),
      );
    }
  ...
  ae = class extends p {
    ...
    async listBuckets(t = {}) {
      var e = this;
      return e.handleOperation(
        async () => await h.post(e.fetch, `${e.url}/ListVectorBuckets`, t, { headers: e.headers }),
      );
    }
    async deleteBucket(t) {
      var e = this;
  ...
  ne = class extends ae {
    ...
    from(t) {
      return new se(this.url, this.headers, t, this.fetch);
    }
    async createBucket(t) {
  ...
  se = class extends te {
    ...
    async createIndex(t) {
      var e = () => super.createIndex,
        r = this;
      return e().call(r, c(c({}, t), {}, { vectorBucketName: r.vectorBucketName }));
    }
    async listIndexes(t = {}) {
    ...
    index(t) {
      return new ce(this.url, this.headers, this.vectorBucketName, t, this.fetch);
    }
  },
  ce = class extends re {
    constructor(t, e, r, a, n) {
      (super(t, e, n), (this.vectorBucketName = r), (this.indexName = a));
    }
    async putVectors(t) {
      ...
      return e().call(
        r,
        c(c({}, t), {}, { vectorBucketName: r.vectorBucketName, indexName: r.indexName }),
      );
    }
    async getVectors(t) {
  ...
  ie = class extends Z {
    ...
    from(t) {
      return new X(this.url, this.headers, t, this.fetch);
    }
    get vectors() {
      return new ne(this.url + "/vector", { headers: this.headers, fetch: this.fetch });
    }
    get analytics() {
      return new ee(this.url + "/iceberg", this.headers, this.fetch);
    }
  };
export { ie as S };
