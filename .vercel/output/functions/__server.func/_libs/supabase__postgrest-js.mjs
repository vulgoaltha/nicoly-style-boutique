const $ = (t) => Math.min(1e3 * 2 ** t, 3e4),
  T = [520, 503],
  j = ["GET", "HEAD", "OPTIONS"];
var L = class extends Error {
  constructor(t) {
    (super(t.message),
      (this.name = "PostgrestError"),
      (this.details = t.details),
      (this.hint = t.hint),
      (this.code = t.code));
  }
  toJSON() {
    return {
      name: this.name,
      message: this.message,
      details: this.details,
      hint: this.hint,
      code: this.code,
    };
  }
};
function A(t, e) {
  return new Promise((r) => {
    if (e?.aborted) {
      r();
      return;
    }
    const s = setTimeout(() => {
      (e?.removeEventListener("abort", l), r());
    }, t);
    function l() {
      (clearTimeout(s), r());
    }
    e?.addEventListener("abort", l);
  });
}
function x(t, e, r, s) {
  return !(!s || r >= 3 || !j.includes(t) || !T.includes(e));
}
var k = class {
    constructor(t) {
      var e, r, s, l, a;
      ((this.shouldThrowOnError = !1),
        (this.retryEnabled = !0),
        (this.method = t.method),
        (this.url = t.url),
        (this.headers = new Headers(t.headers)),
        (this.schema = t.schema),
        (this.body = t.body),
        (this.shouldThrowOnError = (e = t.shouldThrowOnError) !== null && e !== void 0 ? e : !1),
        (this.signal = t.signal),
        (this.isMaybeSingle = (r = t.isMaybeSingle) !== null && r !== void 0 ? r : !1),
        (this.shouldStripNulls = (s = t.shouldStripNulls) !== null && s !== void 0 ? s : !1),
        (this.urlLengthLimit = (l = t.urlLengthLimit) !== null && l !== void 0 ? l : 8e3),
        (this.retryEnabled = (a = t.retry) !== null && a !== void 0 ? a : !0),
        t.fetch ? (this.fetch = t.fetch) : (this.fetch = fetch));
    }
    throwOnError() {
      return ((this.shouldThrowOnError = !0), this);
    }
    stripNulls() {
      if (this.headers.get("Accept") === "text/csv")
        throw new Error("stripNulls() cannot be used with csv()");
      return ((this.shouldStripNulls = !0), this);
    }
    setHeader(t, e) {
      return ((this.headers = new Headers(this.headers)), this.headers.set(t, e), this);
    }
    retry(t) {
      return ((this.retryEnabled = t), this);
    }
    then(t, e) {
      var r = this;
      if (
        (this.schema === void 0 ||
          (["GET", "HEAD"].includes(this.method)
            ? this.headers.set("Accept-Profile", this.schema)
            : this.headers.set("Content-Profile", this.schema)),
        this.method !== "GET" &&
          this.method !== "HEAD" &&
          this.headers.set("Content-Type", "application/json"),
        this.shouldStripNulls)
      ) {
        const i = this.headers.get("Accept");
        i === "application/vnd.pgrst.object+json"
          ? this.headers.set("Accept", "application/vnd.pgrst.object+json;nulls=stripped")
          : (!i || i === "application/json") &&
            this.headers.set("Accept", "application/vnd.pgrst.array+json;nulls=stripped");
      }
      const s = this.fetch;
      let a = (async () => {
        let i = 0;
        for (;;) {
          const o = {};
          (r.headers.forEach((n, d) => {
            o[d] = n;
          }),
            i > 0 && (o["X-Retry-Count"] = String(i)));
          let c;
          try {
            c = await s(r.url.toString(), {
              method: r.method,
              headers: o,
              body: JSON.stringify(r.body, (n, d) => (typeof d == "bigint" ? d.toString() : d)),
              signal: r.signal,
            });
          } catch (n) {
            if (n?.name === "AbortError" || n?.code === "ABORT_ERR" || !j.includes(r.method))
              throw n;
            if (r.retryEnabled && i < 3) {
              const d = $(i);
              (i++, await A(d, r.signal));
              continue;
            }
            throw n;
          }
          if (x(r.method, c.status, i, r.retryEnabled)) {
            var u, h;
            const n =
                (u = (h = c.headers) === null || h === void 0 ? void 0 : h.get("Retry-After")) !==
                  null && u !== void 0
                  ? u
                  : null,
              d = n !== null ? Math.max(0, parseInt(n, 10) || 0) * 1e3 : $(i);
            (await c.text(), i++, await A(d, r.signal));
            continue;
          }
          return await r.processResponse(c);
        }
      })();
      return (
        this.shouldThrowOnError ||
          (a = a.catch((i) => {
            var u;
            let h = "",
              o = "",
              c = "";
            const n = i?.cause;
            if (n) {
              var d, p, f, P;
              const R = (d = n?.message) !== null && d !== void 0 ? d : "",
                b = (p = n?.code) !== null && p !== void 0 ? p : "";
              ((h = `${(f = i?.name) !== null && f !== void 0 ? f : "FetchError"}: ${i?.message}`),
                (h += `

Caused by: ${(P = n?.name) !== null && P !== void 0 ? P : "Error"}: ${R}`),
                b && (h += ` (${b})`),
                n?.stack &&
                  (h += `
${n.stack}`));
            } else {
              var w;
              h = (w = i?.stack) !== null && w !== void 0 ? w : "";
            }
            const g = this.url.toString().length;
            return (
              i?.name === "AbortError" || i?.code === "ABORT_ERR"
                ? ((c = ""),
                  (o = "Request was aborted (timeout or manual cancellation)"),
                  g > this.urlLengthLimit &&
                    (o += `. Note: Your request URL is ${g} characters, which may exceed server limits. If selecting many fields, consider using views. If filtering with large arrays (e.g., .in('id', [many IDs])), consider using an RPC function to pass values server-side.`))
                : (n?.name === "HeadersOverflowError" || n?.code === "UND_ERR_HEADERS_OVERFLOW") &&
                  ((c = ""),
                  (o = "HTTP headers exceeded server limits (typically 16KB)"),
                  g > this.urlLengthLimit &&
                    (o += `. Your request URL is ${g} characters. If selecting many fields, consider using views. If filtering with large arrays (e.g., .in('id', [200+ IDs])), consider using an RPC function instead.`)),
              {
                success: !1,
                error: {
                  message: `${(u = i?.name) !== null && u !== void 0 ? u : "FetchError"}: ${i?.message}`,
                  details: h,
                  hint: o,
                  code: c,
                },
                data: null,
                count: null,
                status: 0,
                statusText: "",
              }
            );
          })),
        a.then(t, e)
      );
    }
    async processResponse(t) {
      var e = this;
      let r = null,
        s = null,
        l = null,
        a = t.status,
        i = t.statusText;
      if (t.ok) {
        var u, h;
        if (e.method !== "HEAD") {
          var o;
          const d = await t.text();
          if (d !== "")
            if (e.headers.get("Accept") === "text/csv") s = d;
            else if (
              e.headers.get("Accept") &&
              !((o = e.headers.get("Accept")) === null || o === void 0) &&
              o.includes("application/vnd.pgrst.plan+text")
            )
              s = d;
            else
              try {
                s = JSON.parse(d);
              } catch {
                if (((r = { message: d }), (s = null), e.shouldThrowOnError))
                  throw new L({ message: d, details: "", hint: "", code: "" });
              }
        }
        const c =
            (u = e.headers.get("Prefer")) === null || u === void 0
              ? void 0
              : u.match(/count=(exact|planned|estimated)/),
          n = (h = t.headers.get("content-range")) === null || h === void 0 ? void 0 : h.split("/");
        (c && n && n.length > 1 && (l = parseInt(n[1])),
          e.isMaybeSingle &&
            Array.isArray(s) &&
            (s.length > 1
              ? ((r = {
                  code: "PGRST116",
                  details: `Results contain ${s.length} rows, application/vnd.pgrst.object+json requires 1 row`,
                  hint: null,
                  message: "JSON object requested, multiple (or no) rows returned",
                }),
                (s = null),
                (l = null),
                (a = 406),
                (i = "Not Acceptable"))
              : s.length === 1
                ? (s = s[0])
                : (s = null)));
      } else {
        const c = await t.text();
        try {
          ((r = JSON.parse(c)),
            Array.isArray(r) && t.status === 404 && ((s = []), (r = null), (a = 200), (i = "OK")));
        } catch {
          t.status === 404 && c === "" ? ((a = 204), (i = "No Content")) : (r = { message: c });
        }
        if (r && e.shouldThrowOnError) throw new L(r);
      }
      return { success: r === null, error: r, data: s, count: l, status: a, statusText: i };
    }
    returns() {
      return this;
    }
    overrideTypes() {
      return this;
    }
  },
  D = class extends k {
    throwOnError() {
      return super.throwOnError();
    }
    select(t) {
      let e = !1;
      const r = (t ?? "*")
        .split("")
        .map((s) => (/\s/.test(s) && !e ? "" : (s === '"' && (e = !e), s)))
        .join("");
      return (
        this.url.searchParams.set("select", r),
        this.headers.append("Prefer", "return=representation"),
        this
      );
    }
    order(t, { ascending: e = !0, nullsFirst: r, foreignTable: s, referencedTable: l = s } = {}) {
      const a = l ? `${l}.order` : "order",
        i = this.url.searchParams.get(a);
      return (
        this.url.searchParams.set(
          a,
          `${i ? `${i},` : ""}${t}.${e ? "asc" : "desc"}${r === void 0 ? "" : r ? ".nullsfirst" : ".nullslast"}`,
        ),
        this
      );
    }
    limit(t, { foreignTable: e, referencedTable: r = e } = {}) {
      const s = typeof r > "u" ? "limit" : `${r}.limit`;
      return (this.url.searchParams.set(s, `${t}`), this);
    }
    range(t, e, { foreignTable: r, referencedTable: s = r } = {}) {
      const l = typeof s > "u" ? "offset" : `${s}.offset`,
        a = typeof s > "u" ? "limit" : `${s}.limit`;
      return (
        this.url.searchParams.set(l, `${t}`),
        this.url.searchParams.set(a, `${e - t + 1}`),
        this
      );
    }
    abortSignal(t) {
      return ((this.signal = t), this);
    }
    single() {
      return (this.headers.set("Accept", "application/vnd.pgrst.object+json"), this);
    }
    maybeSingle() {
      return ((this.isMaybeSingle = !0), this);
    }
    csv() {
      return (this.headers.set("Accept", "text/csv"), this);
    }
    geojson() {
      return (this.headers.set("Accept", "application/geo+json"), this);
    }
    explain({
      analyze: t = !1,
      verbose: e = !1,
      settings: r = !1,
      buffers: s = !1,
      wal: l = !1,
      format: a = "text",
    } = {}) {
      var i;
      const u = [
          t ? "analyze" : null,
          e ? "verbose" : null,
          r ? "settings" : null,
          s ? "buffers" : null,
          l ? "wal" : null,
        ]
          .filter(Boolean)
          .join("|"),
        h = (i = this.headers.get("Accept")) !== null && i !== void 0 ? i : "application/json";
      return (
        this.headers.set("Accept", `application/vnd.pgrst.plan+${a}; for="${h}"; options=${u};`),
        a === "json" ? this : this
      );
    }
    rollback() {
      return (this.headers.append("Prefer", "tx=rollback"), this);
    }
    returns() {
      return this;
    }
    maxAffected(t) {
      return (
        this.headers.append("Prefer", "handling=strict"),
        this.headers.append("Prefer", `max-affected=${t}`),
        this
      );
    }
  };
const O = new RegExp("[,()]");
var m = class extends D {
    throwOnError() {
      return super.throwOnError();
    }
    eq(t, e) {
      return (this.url.searchParams.append(t, `eq.${e}`), this);
    }
    neq(t, e) {
      return (this.url.searchParams.append(t, `neq.${e}`), this);
    }
    gt(t, e) {
      return (this.url.searchParams.append(t, `gt.${e}`), this);
    }
    gte(t, e) {
      return (this.url.searchParams.append(t, `gte.${e}`), this);
    }
    lt(t, e) {
      return (this.url.searchParams.append(t, `lt.${e}`), this);
    }
    lte(t, e) {
      return (this.url.searchParams.append(t, `lte.${e}`), this);
    }
    like(t, e) {
      return (this.url.searchParams.append(t, `like.${e}`), this);
    }
    likeAllOf(t, e) {
      return (this.url.searchParams.append(t, `like(all).{${e.join(",")}}`), this);
    }
    likeAnyOf(t, e) {
      return (this.url.searchParams.append(t, `like(any).{${e.join(",")}}`), this);
    }
    ilike(t, e) {
      return (this.url.searchParams.append(t, `ilike.${e}`), this);
    }
    ilikeAllOf(t, e) {
      return (this.url.searchParams.append(t, `ilike(all).{${e.join(",")}}`), this);
    }
    ilikeAnyOf(t, e) {
      return (this.url.searchParams.append(t, `ilike(any).{${e.join(",")}}`), this);
    }
    regexMatch(t, e) {
      return (this.url.searchParams.append(t, `match.${e}`), this);
    }
    regexIMatch(t, e) {
      return (this.url.searchParams.append(t, `imatch.${e}`), this);
    }
    is(t, e) {
      return (this.url.searchParams.append(t, `is.${e}`), this);
    }
    isDistinct(t, e) {
      return (this.url.searchParams.append(t, `isdistinct.${e}`), this);
    }
    in(t, e) {
      const r = Array.from(new Set(e))
        .map((s) => (typeof s == "string" && O.test(s) ? `"${s}"` : `${s}`))
        .join(",");
      return (this.url.searchParams.append(t, `in.(${r})`), this);
    }
    notIn(t, e) {
      const r = Array.from(new Set(e))
        .map((s) => (typeof s == "string" && O.test(s) ? `"${s}"` : `${s}`))
        .join(",");
      return (this.url.searchParams.append(t, `not.in.(${r})`), this);
    }
    contains(t, e) {
      return (
        typeof e == "string"
          ? this.url.searchParams.append(t, `cs.${e}`)
          : Array.isArray(e)
            ? this.url.searchParams.append(t, `cs.{${e.join(",")}}`)
            : this.url.searchParams.append(t, `cs.${JSON.stringify(e)}`),
        this
      );
    }
    containedBy(t, e) {
      return (
        typeof e == "string"
          ? this.url.searchParams.append(t, `cd.${e}`)
          : Array.isArray(e)
            ? this.url.searchParams.append(t, `cd.{${e.join(",")}}`)
            : this.url.searchParams.append(t, `cd.${JSON.stringify(e)}`),
        this
      );
    }
    rangeGt(t, e) {
      return (this.url.searchParams.append(t, `sr.${e}`), this);
    }
    rangeGte(t, e) {
      return (this.url.searchParams.append(t, `nxl.${e}`), this);
    }
    rangeLt(t, e) {
      return (this.url.searchParams.append(t, `sl.${e}`), this);
    }
    rangeLte(t, e) {
      return (this.url.searchParams.append(t, `nxr.${e}`), this);
    }
    rangeAdjacent(t, e) {
      return (this.url.searchParams.append(t, `adj.${e}`), this);
    }
    overlaps(t, e) {
      return (
        typeof e == "string"
          ? this.url.searchParams.append(t, `ov.${e}`)
          : this.url.searchParams.append(t, `ov.{${e.join(",")}}`),
        this
      );
    }
    textSearch(t, e, { config: r, type: s } = {}) {
      let l = "";
      s === "plain" ? (l = "pl") : s === "phrase" ? (l = "ph") : s === "websearch" && (l = "w");
      const a = r === void 0 ? "" : `(${r})`;
      return (this.url.searchParams.append(t, `${l}fts${a}.${e}`), this);
    }
    match(t) {
      return (
        Object.entries(t)
          .filter(([e, r]) => r !== void 0)
          .forEach(([e, r]) => {
            this.url.searchParams.append(e, `eq.${r}`);
          }),
        this
      );
    }
    not(t, e, r) {
      return (this.url.searchParams.append(t, `not.${e}.${r}`), this);
    }
    or(t, { foreignTable: e, referencedTable: r = e } = {}) {
      const s = r ? `${r}.or` : "or";
      return (this.url.searchParams.append(s, `(${t})`), this);
    }
    filter(t, e, r) {
      return (this.url.searchParams.append(t, `${e}.${r}`), this);
    }
  },
  N = class {
    constructor(t, { headers: e = {}, schema: r, fetch: s, urlLengthLimit: l = 8e3, retry: a }) {
      ((this.url = t),
        (this.headers = new Headers(e)),
        (this.schema = r),
        (this.fetch = s),
        (this.urlLengthLimit = l),
        (this.retry = a));
    }
    cloneRequestState() {
      return { url: new URL(this.url.toString()), headers: new Headers(this.headers) };
    }
    select(t, e) {
      const { head: r = !1, count: s } = e ?? {},
        l = r ? "HEAD" : "GET";
      let a = !1;
      const i = (t ?? "*")
          .split("")
          .map((o) => (/\s/.test(o) && !a ? "" : (o === '"' && (a = !a), o)))
          .join(""),
        { url: u, headers: h } = this.cloneRequestState();
      return (
        u.searchParams.set("select", i),
        s && h.append("Prefer", `count=${s}`),
        new m({
          method: l,
          url: u,
          headers: h,
          schema: this.schema,
          fetch: this.fetch,
          urlLengthLimit: this.urlLengthLimit,
          retry: this.retry,
        })
      );
    }
    insert(t, { count: e, defaultToNull: r = !0 } = {}) {
      var s;
      const l = "POST",
        { url: a, headers: i } = this.cloneRequestState();
      if (
        (e && i.append("Prefer", `count=${e}`),
        r || i.append("Prefer", "missing=default"),
        Array.isArray(t))
      ) {
        const u = t.reduce((h, o) => h.concat(Object.keys(o)), []);
        if (u.length > 0) {
          const h = [...new Set(u)].map((o) => `"${o}"`);
          a.searchParams.set("columns", h.join(","));
        }
      }
      return new m({
        method: l,
        url: a,
        headers: i,
        schema: this.schema,
        body: t,
        fetch: (s = this.fetch) !== null && s !== void 0 ? s : fetch,
        urlLengthLimit: this.urlLengthLimit,
        retry: this.retry,
      });
    }
    upsert(t, { onConflict: e, ignoreDuplicates: r = !1, count: s, defaultToNull: l = !0 } = {}) {
      var a;
      const i = "POST",
        { url: u, headers: h } = this.cloneRequestState();
      if (
        (h.append("Prefer", `resolution=${r ? "ignore" : "merge"}-duplicates`),
        e !== void 0 && u.searchParams.set("on_conflict", e),
        s && h.append("Prefer", `count=${s}`),
        l || h.append("Prefer", "missing=default"),
        Array.isArray(t))
      ) {
        const o = t.reduce((c, n) => c.concat(Object.keys(n)), []);
        if (o.length > 0) {
          const c = [...new Set(o)].map((n) => `"${n}"`);
          u.searchParams.set("columns", c.join(","));
        }
      }
      return new m({
        method: i,
        url: u,
        headers: h,
        schema: this.schema,
        body: t,
        fetch: (a = this.fetch) !== null && a !== void 0 ? a : fetch,
        urlLengthLimit: this.urlLengthLimit,
        retry: this.retry,
      });
    }
    update(t, { count: e } = {}) {
      var r;
      const s = "PATCH",
        { url: l, headers: a } = this.cloneRequestState();
      return (
        e && a.append("Prefer", `count=${e}`),
        new m({
          method: s,
          url: l,
          headers: a,
          schema: this.schema,
          body: t,
          fetch: (r = this.fetch) !== null && r !== void 0 ? r : fetch,
          urlLengthLimit: this.urlLengthLimit,
          retry: this.retry,
        })
      );
    }
    delete({ count: t } = {}) {
      var e;
      const r = "DELETE",
        { url: s, headers: l } = this.cloneRequestState();
      return (
        t && l.append("Prefer", `count=${t}`),
        new m({
          method: r,
          url: s,
          headers: l,
          schema: this.schema,
          fetch: (e = this.fetch) !== null && e !== void 0 ? e : fetch,
          urlLengthLimit: this.urlLengthLimit,
          retry: this.retry,
        })
      );
    }
  };
function y(t) {
  "@babel/helpers - typeof";
  return (
    (y =
      typeof Symbol == "function" && typeof Symbol.iterator == "symbol"
        ? function (e) {
            return typeof e;
          }
        : function (e) {
            return e &&
              typeof Symbol == "function" &&
              e.constructor === Symbol &&
              e !== Symbol.prototype
              ? "symbol"
              : typeof e;
          }),
    y(t)
  );
}
function H(t, e) {
  if (y(t) != "object" || !t) return t;
  var r = t[Symbol.toPrimitive];
  if (r !== void 0) {
    var s = r.call(t, e);
    if (y(s) != "object") return s;
    throw new TypeError("@@toPrimitive must return a primitive value.");
  }
  return (e === "string" ? String : Number)(t);
}
function q(t) {
  var e = H(t, "string");
  return y(e) == "symbol" ? e : e + "";
}
function _(t, e, r) {
  return (
    (e = q(e)) in t
      ? Object.defineProperty(t, e, { value: r, enumerable: !0, configurable: !0, writable: !0 })
      : (t[e] = r),
    t
  );
}
function S(t, e) {
  var r = Object.keys(t);
  if (Object.getOwnPropertySymbols) {
    var s = Object.getOwnPropertySymbols(t);
    (e &&
      (s = s.filter(function (l) {
        return Object.getOwnPropertyDescriptor(t, l).enumerable;
      })),
      r.push.apply(r, s));
  }
  return r;
}
function v(t) {
  for (var e = 1; e < arguments.length; e++) {
    var r = arguments[e] != null ? arguments[e] : {};
    e % 2
      ? S(Object(r), !0).forEach(function (s) {
          _(t, s, r[s]);
        })
      : Object.getOwnPropertyDescriptors
        ? Object.defineProperties(t, Object.getOwnPropertyDescriptors(r))
        : S(Object(r)).forEach(function (s) {
            Object.defineProperty(t, s, Object.getOwnPropertyDescriptor(r, s));
          });
  }
  return t;
}
var C = class E {
  constructor(
    e,
    { headers: r = {}, schema: s, fetch: l, timeout: a, urlLengthLimit: i = 8e3, retry: u } = {},
  ) {
    ((this.url = e),
      (this.headers = new Headers(r)),
      (this.schemaName = s),
      (this.urlLengthLimit = i));
    const h = l ?? globalThis.fetch;
    (a !== void 0 && a > 0
      ? (this.fetch = (o, c) => {
          const n = new AbortController(),
            d = setTimeout(() => n.abort(), a),
            p = c?.signal;
          if (p) {
            if (p.aborted) return (clearTimeout(d), h(o, c));
            const f = () => {
              (clearTimeout(d), n.abort());
            };
            return (
              p.addEventListener("abort", f, { once: !0 }),
              h(o, v(v({}, c), {}, { signal: n.signal })).finally(() => {
                (clearTimeout(d), p.removeEventListener("abort", f));
              })
            );
          }
          return h(o, v(v({}, c), {}, { signal: n.signal })).finally(() => clearTimeout(d));
        })
      : (this.fetch = h),
      (this.retry = u));
  }
  from(e) {
    if (!e || typeof e != "string" || e.trim() === "")
      throw new Error("Invalid relation name: relation must be a non-empty string.");
    return new N(new URL(`${this.url}/${e}`), {
      headers: new Headers(this.headers),
      schema: this.schemaName,
      fetch: this.fetch,
      urlLengthLimit: this.urlLengthLimit,
      retry: this.retry,
    });
  }
  schema(e) {
    return new E(this.url, {
      headers: this.headers,
      schema: e,
      fetch: this.fetch,
      urlLengthLimit: this.urlLengthLimit,
      retry: this.retry,
    });
  }
  rpc(e, r = {}, { head: s = !1, get: l = !1, count: a } = {}) {
    var i;
    let u;
    const h = new URL(`${this.url}/rpc/${e}`);
    let o;
    const c = (p) => p !== null && typeof p == "object" && (!Array.isArray(p) || p.some(c)),
      n = s && Object.values(r).some(c);
    n
      ? ((u = "POST"), (o = r))
      : s || l
        ? ((u = s ? "HEAD" : "GET"),
          Object.entries(r)
            .filter(([p, f]) => f !== void 0)
            .map(([p, f]) => [p, Array.isArray(f) ? `{${f.join(",")}}` : `${f}`])
            .forEach(([p, f]) => {
              h.searchParams.append(p, f);
            }))
        : ((u = "POST"), (o = r));
    const d = new Headers(this.headers);
    return (
      n
        ? d.set("Prefer", a ? `count=${a},return=minimal` : "return=minimal")
        : a && d.set("Prefer", `count=${a}`),
      new m({
        method: u,
        url: h,
        headers: d,
        schema: this.schemaName,
        body: o,
        fetch: (i = this.fetch) !== null && i !== void 0 ? i : fetch,
        urlLengthLimit: this.urlLengthLimit,
        retry: this.retry,
      })
    );
  }
};
export { C as P };
