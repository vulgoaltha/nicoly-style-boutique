...
function T(e, t, a) {
  const s = new URL(t, e);
  if (a) for (const [p, n] of Object.entries(a)) n !== void 0 && s.searchParams.set(p, n);
  return s.toString();
}
async function g(e) {
...
function w(e) {
  ...
  return {
    async request({ method: a, path: s, query: p, body: n, headers: b }) {
      const f = T(e.baseUrl, s, p),
        y = await g(e.auth),
        r = await t(f, {
          method: a,
          headers: { ...(n ? { "Content-Type": "application/json" } : {}), ...y, ...b },
          body: n ? JSON.stringify(n) : void 0,
        }),
        l = await r.text(),
        u = (r.headers.get("content-type") || "").includes("application/json"),
        m = u && l ? JSON.parse(l) : l;
      if (!r.ok) {
        const d = u ? m : void 0,
          h = d?.error;
        throw new i(h?.message ?? `Request failed with status ${r.status}`, {
          status: r.status,
          icebergType: h?.type,
          icebergCode: h?.code,
          details: d,
      ...
      return { status: r.status, headers: r.headers, data: m };
    },
  };
}
function o(e) {
...
var x = class {
  ...
  async createNamespace(e, t) {
    const a = { namespace: e.namespace, properties: t?.properties };
    return (
      await this.client.request({ method: "POST", path: `${this.prefix}/namespaces`, body: a })
    ).data;
  }
  ...
  async namespaceExists(e) {
    try {
      ...
      if (t instanceof i && t.status === 404) return !1;
      throw t;
    }
  }
  async createNamespaceIfNotExists(e, t) {
    try {
      ...
      if (a instanceof i && a.status === 409) return;
      throw a;
    }
  }
};
...
var E = class {
    ...
    async listTables(e) {
      return (
        await this.client.request({
          method: "GET",
          path: `${this.prefix}/namespaces/${c(e.namespace)}/tables`,
        })
      ).data.identifiers;
    }
    ...
    async updateTable(e, t) {
      const a = await this.client.request({
        method: "POST",
        path: `${this.prefix}/namespaces/${c(e.namespace)}/tables/${e.name}`,
        body: t,
      });
      return { "metadata-location": a.data["metadata-location"], metadata: a.data.metadata };
    }
    async dropTable(e, t) {
      await this.client.request({
        method: "DELETE",
    ...
    async tableExists(e) {
      ...
      try {
        ...
        if (a instanceof i && a.status === 404) return !1;
        throw a;
      }
    }
    async createTableIfNotExists(e, t) {
      try {
        ...
        if (a instanceof i && a.status === 409)
  ...
  N = class {
    constructor(e) {
      ...
      const a = e.baseUrl.endsWith("/") ? e.baseUrl : `${e.baseUrl}/`;
      ((this.client = w({ baseUrl: a, auth: e.auth, fetchImpl: e.fetch })),
        (this.accessDelegation = e.accessDelegation?.join(",")),
        (this.namespaceOps = new x(this.client, t)),
        (this.tableOps = new E(this.client, t, this.accessDelegation)));
    }
    async listNamespaces(e) {
