...
test.describe("Auditoria Completa de Catálogo", () => {
  ...
  test("1. Auditoria de Categorias", async ({ page }) => {
    ...
    if (btnCount > 0) {
      ...
      const titleInput = page.locator(
        'input[name="title"], input[name="name"], input[placeholder*="nome" i], input[placeholder*="título" i]',
      );
  ...
  test("2. Auditoria de Coleções", async ({ page }) => {
    ...
    if (btnCount > 0) {
      ...
      const nameInput = page.locator(
        'input[name="name"], input[name="title"], input[placeholder*="nome" i]',
      );
  ...
  test("3. Auditoria de Banners", async ({ page }) => {
    ...
    if (btnCount > 0) {
      ...
      const titleInput = page.locator(
        'input[name="title"], input[name="name"], input[placeholder*="título" i]',
      );
      if ((await titleInput.count()) > 0) {
        await titleInput.first().fill("Banner Audit E2E");

        const ctaInput = page.locator(
          'input[name="cta_text"], input[placeholder*="CTA" i], input[placeholder*="botão" i]',
        );
        ...
        const linkInput = page.locator(
          'input[name="link"], input[name="url"], input[placeholder*="link" i]',
        );
  ...
  test("4. Auditoria de Produtos", async ({ page }) => {
    ...
    if (btnCount > 0) {
      ...
      const salePriceInput = page.locator(
        'input[name="sale_price"], input[name="promo_price"], input[placeholder*="promo" i]',
      );
      ...
      const stockInput = page.locator(
        'input[name="stock"], input[name="quantity"], input[placeholder*="estoque" i]',
      );
