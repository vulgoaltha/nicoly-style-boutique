...
**Prioridade 2 (Verificar no Supabase Dashboard):** 3. Verificar se o trigger `on_auth_user_created` esta ativo via SQL Editor 4. Executar o seed inicial para ter dados de teste

**Prioridade 3 (Melhorias):** 5. Considerar adicionar `ON DELETE CASCADE` em `profiles.id -> auth.users.id` para limpeza automatica 6. Verificar se a `SUPABASE_SERVICE_ROLE_KEY` esta configurada no .env
